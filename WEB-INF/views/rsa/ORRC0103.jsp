<%--
/*---------------------------------------------------------------------------
 Program ID   : ORRC0103.jsp
 Program name : 리스크풀관리 수정/조회
 Description  : 화면정의서 RCSA-02.8
 Programer    : 박승윤
...
Vector vRskPoolLst= CommUtil.getResultVector(request, "grp01", 0, "unit02", 0, "vList");
if(vRskPoolLst==null) vRskPoolLst = new Vector();
...
HashMap hRskPoolMap = null;
if(vRskPoolLst.size()>0){
	hRskPoolMap = (HashMap)vRskPoolLst.get(0);
}else{
	hRskPoolMap = new HashMap();
}

String updateYn = request.getParameter("updateYn");
char hq_yn = ((String)hRskPoolMap.get("prss")).charAt(0);
System.out.println(hq_yn);
%>
<!DOCTYPE html>
<html lang="ko">
<head>
	...
	<script>
		...
		function initIBSheet1() {
			...
			initData1.Cols = [
				...
				{Header:"통제내용",Type:"Text",Width:560,Align:"Left",SaveName:"cp_cntn",MinWidth:260,Wrap:1,EditLen:800}
			];
		...
		function initIBSheet2() {
			...
			initData2.Cols = [
				{Header:"선택",Type:"CheckBox",Width:70,Align:"Left",SaveName:"ischeck",MinWidth:50, Hidden:true},
		...
		function mySheet2_OnSearchEnd(code, message) {
			...
			if (mySheet2.GetDataFirstRow() > 0)
				for(var nCnt=mySheet2.GetDataFirstRow(); nCnt <= mySheet2.GetDataLastRow(); nCnt++){
					var totLssam = "0";
					if(mySheet2.GetCellValue(nCnt,"tot_lssam") != "0" && mySheet2.GetCellValue(nCnt,"tot_lssam") != ""){
			...
			WP.load(url, inputData,
				{
					success: function(result){
						if(result!='undefined' && result.rtnCode=="S") {
							...
							parent.$("#ifrRskMod").attr("src","about:blank");
...
<body>
	...
	<div id="" class="popup modal block" >
		<div class="p_frame w1100">
			<div class="p_head">
				<h3 class="title">리스크풀 <%if("".equals(updateYn) || updateYn == null){%>수정<%}else{%>상세조회<%}%></h3>
			</div>
			<div class="p_body">
				<div class="p_wrap">
					<form name="ormsForm" method="post">
						...
						<input type="hidden" id="prss" name="prss" value="<%=(String)hRskPoolMap.get("prss")%>" />
						<input type="hidden" id="emrk" name="emrk" value="<%=(String)hRskPoolMap.get("emrk")%>" />
						<input type="hidden" id="cas" name="cas" value="<%=(String)hRskPoolMap.get("cas")%>" />
						<input type="hidden" id="hpn" name="hpn" value="<%=(String)hRskPoolMap.get("hpn")%>" />
						<input type="hidden" id="ifn" name="ifn" value="<%=(String)hRskPoolMap.get("ifn")%>" />
						<input type="hidden" id="brc" name="brc" value="<%=(String)hRskPoolMap.get("brc")%>" />
						<input type="hidden" id="team_cd" name="team_cd" value="<%=(String)hRskPoolMap.get("team_cd")%>" />
						<input type="hidden" id="rkp_tpc" name="rkp_tpc" value="" />
						<input type="hidden" id="apl_aflco_dsc" name="apl_aflco_dsc" value="04" />
						<div id="ctl_area"></div>
										...
										<table>
											...
											<tbody>
												<tr>
													...
													<td>
														<div class="input-group">
															<input type="text" class="form-control" id="prssnm1" value="<%=(String)hRskPoolMap.get("prssnm1")%>"  readonly>
															<%if("".equals(updateYn) || updateYn == null){%>
												...
												<tr>
													<th>중분류</th>
													<td>
														<input type="text" class="form-control" id="prssnm2" value="<%=(String)hRskPoolMap.get("prssnm2")%>"  readonly>
													</td>
												</tr>
												<tr>
													<th>단위업무</th>
													<td>
														<input type="text" class="form-control" id="prssnm3" value="<%=(String)hRskPoolMap.get("prssnm3")%>"  readonly>
													</td>
												</tr>
												<tr>
													<th>세부업무</th>
													<td>
														<input type="text" class="form-control" id="prssnm4" value="<%=(String)hRskPoolMap.get("prssnm4")%>"  readonly>
													</td>
												</tr>
											</tbody>
										</table>
										...
										<table>
											...
											<tbody>
												<tr>
													...
													<td>
														<div class="input-group">
															<input type="text" class="form-control" id="emrknm1" value="<%=(String)hRskPoolMap.get("emrknm1")%>"  readonly>
															<%if("".equals(updateYn) || updateYn == null){%>
															<span class="input-group-btn">
												...
												<tr>
													<th>Lv.2</th>
													<td>
														<input type="text" class="form-control" id="emrknm2" value="<%=(String)hRskPoolMap.get("emrknm2")%>"  readonly>
													</td>
												</tr>
											</tbody>
										</table>
										...
										<table>
											...
											<tbody>
												<tr>
													...
													<td>
														<div class="input-group">
															<input type="text" class="form-control" id="ifnnm1" value="<%=(String)hRskPoolMap.get("ifnnm1")%>"  readonly>
															<%if("".equals(updateYn) || updateYn == null){%>
															<span class="input-group-btn">
												...
												<tr>
													<th>Lv 2</th>
													<td>
														<input type="text" class="form-control" id="ifnnm2" value="<%=(String)hRskPoolMap.get("ifnnm2")%>"  readonly>
													</td>
												</tr>
											</tbody>
										</table>
										...
										<table>
											...
											<tbody>
												<tr>
													...
													<td>
														<div class="input-group">
															<input type="text" class="form-control" id="casnm1" value="<%=(String)hRskPoolMap.get("casnm1")%>"  readonly>
															<%if("".equals(updateYn) || updateYn == null){%>
															<span class="input-group-btn">
												...
												<tr>
													<th>Lv 2</th>
													<td>
														<input type="text" class="form-control" id="casnm2" value="<%=(String)hRskPoolMap.get("casnm2")%>"  readonly>
													</td>
												</tr>
												<tr>
													<th>Lv 3</th>
													<td>
														<input type="text" class="form-control" id="casnm3" value="<%=(String)hRskPoolMap.get("casnm3")%>"  readonly>
													</td>
												</tr>
											</tbody>
										</table>
										...
										<table>
											...
											<tbody>
												<tr>
													...
													<td>
														<div class="input-group">
															<input type="text" class="form-control" id="hpnnm1" value="<%=(String)hRskPoolMap.get("hpnnm1")%>"  readonly>
															<%if("".equals(updateYn) || updateYn == null){%>
															<span class="input-group-btn">
												...
												<tr>
													<th>Lv 2</th>
													<td>
														<input type="text" class="form-control" id="hpnnm2" value="<%=(String)hRskPoolMap.get("hpnnm2")%>"  readonly>
													</td>
												</tr>
												<tr>
													<th>Lv 3</th>
													<td>
														<input type="text" class="form-control" id="hpnnm3" value="<%=(String)hRskPoolMap.get("hpnnm3")%>"  readonly>
													</td>
												</tr>
											</tbody>
										</table>
										...
										<table>
											...
											<tbody>
												<tr>
													<th>부서</th>
													<td>
															<input type="text" class="form-control" id="brnm" value="<%=(String)hRskPoolMap.get("brnm")%>"  readonly>										
													</td>
												</tr>
												<%if("H".equals(String.valueOf(((String)hRskPoolMap.get("prss")).charAt(0)))){%>
												<tr>
													<th>팀</th>
													<td>
														<div class="input-group">
															<input type="text" class="form-control" id="target_evl_team" value="<%=(String)hRskPoolMap.get("target_evl_team")%>"  readonly>
															<%if("".equals(updateYn) || updateYn == null){%>
															<span class="input-group-btn">
								...
								<table>
									...
									<tbody>
										<tr>
											<th>RP ID</th>
											<td>
												<input type="text" class="form-control" id="rkp_id" name="rkp_id" value="<%=(String)hRskPoolMap.get("rkp_id")%>"  readonly>
											</td>
										</tr>
										<tr>
											<th>내용</th>
											<td>
												<textarea id="rk_isc_cntn" name="rk_isc_cntn" cols="100" rows="5" class="form-control textarea" <%if("N".equals(updateYn)){%>readonly<%}%> ><%=StringUtil.htmlEscape((String)hRskPoolMap.get("rk_isc_cntn"),false,false)%></textarea>
											</td>
										</tr>
									</tbody>
								</table>
	...
	<div id='winLoss' class='popup modal'>
		<iframe id='ifrLoss' src="about:blank" name='ifrLoss'></iframe>
	</div>
	<div id='winKri' class='popup modal'>
		<iframe id='ifrKri' src="about:blank" name='ifrKri'></iframe>
	</div>
	...
	<script>
		...
		function prssSearchEnd(bsn_prss_c, bsn_prsnm
			...
			if(bsn_prss_c.charAt(0)!=$("#prss").val().charAt(0))
			{
				if($("#prss").val().charAt(0) == "B")
				{
					alert("영업점 리스크 사례 입니다.영업점 프로세스를 선택하여 주세요.");
				}
				else if($("#prss").val().charAt(0) == "H")
				{
					alert("본부부서 리스크 사례 입니다.본부부서 프로세스를 선택하여 주세요.");
				}
			}else
			{
				$("#prss").val(bsn_prss_c);
	...
	$(document).ready(function(){
		...
		*/		
		...
		var i = 0;
		$('.p_body').on('scroll',function(event){
			if(i < 1) $('.p_body').scrollTop(0);
			i = i + 1;
		});
	});
