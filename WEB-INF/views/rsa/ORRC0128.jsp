<%--
/*---------------------------------------------------------------------------
 Program ID   : ORRC0128.jsp
 Program name : 손실사건 리스크사례 연결등록
 Description  : 화면정의서 RCSA-18.1
 Programer    : 박승윤
...
<script language="javascript">
		$(document).ready(function(){
			...
			$("form[name=ormsForm] [name=process_id]").val("ORRC012803");
		...
		function doAction(sAction) {
			...
			WP.setParameter("process_id", "ORRC012802");
			WP.setForm(f);
			...
			WP.load(url, inputData,{
				success: function(result){
				  ...
				  if(result!='undefined' && result.rtnCode=="0") 
				  {
					...
					if(rList.length>0){                                  
					$("#dept_brnm").text(rList[0].dept_brnm);
					$("#brnm").text(rList[0].brnm);
					$("#oprk_rki_id").text(rList[0].oprk_rki_id);
					$("#oprk_rkinm").text(rList[0].oprk_rkinm);
					$("#rki_obv_cntn").text(rList[0].rki_obv_cntn);
					$("#rki_def_cntn").text(rList[0].rki_def_cntn);
					$("#rki_unt_c").text(rList[0].rki_unt_c);
					$("#rki_unt_nm").text(rList[0].rki_unt_nm);
					$("#rpt_fq_dsc").text(rList[0].rpt_fq_dsc);
					$("#rpt_fq_dsnm").text(rList[0].rpt_fq_dsnm);
					$("#com_col_psb_yn").text(rList[0].com_col_psb_yn);
					$("#kri_yn").text(rList[0].kri_yn);
...
function copyRkp(){
		...
		function doSave() {
			...
			mySheet2.DoSave(url, { Param : "method=Main&commkind=rsa&process_id=ORRC012804&rki_id="+$("#rki_id").val(), Col : 0 });
		}
...
<body>
	...
	<article class="popup modal block">
		<div class="p_frame w1200">	
			<div class="p_head">
				<h1 class="title">KRI-리스크사례 연결등록</h1>
			</div>
			<form name="ormsForm">
			...
			<input type="hidden" id="rki_id" name="rki_id" value="<%=form.get("rki_id")%>"/>
				...
				<div class="p_wrap">
					...
					<section class="box box-grid">
						<div class="box-header">
							<h2 class="box-title">KRI 지표 개요</h2>
						</div>
						<div class="wrap-table">
							<table>
								...
								<thead>
									<tr>
										<th scope="col" colspan="2">평가부서</th>
										<th scope="col" rowspan="2">KRI ID</th>
										<th scope="col" rowspan="2">지표명</th>
										<th scope="col" rowspan="2">지표목적</th>
										<th scope="col" rowspan="2">지표산식</th>
										<th scope="col" rowspan="2">단위</th>
										<th scope="col" rowspan="2">수집주기</th>
										<th scope="col" rowspan="2">전산KRI여부</th>
										<th scope="col" rowspan="2">평가대상여부</th>
									</tr>
									<tr>
										<th scope="col">부서</th>
										<th scope="col">팀</th>
								...
								<tbody class="center">
									<tr>
										<td id="dept_brnm"></td>
										<td id="brnm"></td>
										<td id="oprk_rki_id"></td>
										<td id="oprk_rkinm" class="left"></td>
										<td id="rki_obv_cntn" class="left"></td>
										<td id="rki_def_cntn"></td>
										<td id="rki_unt_nm"></td>
										<td id="rpt_fq_dsnm"></td>
										<td id="com_col_psb_yn"></td>
										<td id="kri_yn"></td>
									</tr>
								</tbody>
							</table>
