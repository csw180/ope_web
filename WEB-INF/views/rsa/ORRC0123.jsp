...
$(document).ready(function(){
			...
			$("#sch_exe_yn").change(function(){
				if($("#sch_exe_yn").val()=='Y'){
					$("#calendarButton3").show();	
				}
				else if($("#sch_exe_yn").val()=='N'){
					$("#calendarButton3").hide();
					$("#act_cpl_dt").val('');
				}
			})
			
			if($("#rcsa_menu_dsc").val()=='2' && $("#rcsa_act_dcz_stsc").val()<='23')
			{
				$("#exe_rzt_cntn").attr("readonly",false);
				$("#sch_exe_yn").attr("disabled",false);
			}
		// ibsheet 초기화
		initIBSheet1();
		initIBSheet2();
...
function DoSearch() {
		...
		$("form[name=ormsForm] [name=process_id]").val("ORRC012302");
		...
		$("form[name=ormsForm] [name=method]").val("Main");
		$("form[name=ormsForm] [name=commkind]").val("rsa");
		$("form[name=ormsForm] [name=process_id]").val("ORRC012303");
...
function DoSearch2() {
			...
			WP.setParameter("method", "Main");
			WP.setParameter("commkind", "rsa");
			WP.setParameter("process_id", "ORRC012304");
			WP.setForm(f);
			...
			WP.load(url, inputData,{
				success: function(result){
				  ...
				  if(result!='undefined' && result.rtnCode=="0") 
				  {
					...
					$("#exe_rzt_cntn").text(rList[0].exe_rzt_cntn);
					$("#act_cpl_dt").val(rList[0].act_cpl_dt);
					...
					if(rList[0].act_cpl_dt ==""){
						$("#sch_exe_yn").val('N');
					}else if(rList[0].act_cpl_dt!=""){
						$("#sch_exe_yn").val('Y');
						if($("#rcsa_menu_dsc").val()=="2" && $("#rcsa_act_dcz_stsc").val()<='23'){
							$("#calendarButton3").show();	
						}
...
function DoSearch3() {
			...
			WP.setParameter("method", "Main");
			WP.setParameter("commkind", "rsa");
			WP.setParameter("process_id", "ORRC012306");
			WP.setForm(f);
...
function DoSearch4() {
			...
			WP.setParameter("method", "Main");
			WP.setParameter("commkind", "rsa");
			WP.setParameter("process_id", "ORRC012307");
			WP.setForm(f);
...
function save(sAction){
			...
			f.act_cpl_dt.value =  f.act_cpl_dt.value.replace(/-/g,"");
			...
			if( f.exe_rzt_cntn.value == ''){
				alert("이행결과 종합 의견을 입력해 주세요")
				return;
			}
			
			if( f.sch_exe_yn.value == 'Y' && f.act_cpl_dt.value==''){
				alert("이행 완료일을 입력해 주세요.")
				return;
			}
			...
			if(f.act_cpl_dt.value > f.act_ed_dt.value){
				alert("이행 완료일은 이행 완료과 같거나 작아야 합니다.")
				return;
			}	
...
//입력값 체크
		...
		var rcsa_act_dcz_stsc = $("#rcsa_act_dcz_stsc").val(); 
			...
			if( rcsa_menu_dsc == 2){
			    ...
			    }else
			    {
			 	...
			 	if( rcsa_act_dcz_stsc > "24" ){
					 alert("이미 결재 완료한 건 입니다.");
					 return false;
				}else if(rcsa_act_dcz_stsc < "24")
				{
					 alert("결재요청 되지 않았습니다.");
					 return false;
				}
			    ...
			    $("#dcz_objr_emp_auth").val("'002'");	 
			 	...
			 	if( rcsa_act_dcz_stsc > "33" ){
					 alert("이미 결재 완료한 건 입니다.");
					 return false;
				}else if(rcsa_act_dcz_stsc < "33")
				{
			    ...
			    $("#dcz_objr_emp_auth").val("'009'");	 	 
			 	...
			 	if( rcsa_act_dcz_stsc > "34" ){
					 alert("이미 결재 완료한 건 입니다.");
					 return false;
				}else if(rcsa_act_dcz_stsc < "34")
				{
	...
	function doSave() {
			...
			WP.setParameter("method", "Main");
			WP.setParameter("commkind", "rsa");
			WP.setParameter("process_id", "ORRC012305");
			...
			<div class="box row">
				<div class="col">
					...
					<div class="col">
							<section class="box box-grid">
								<div class="box-header">
									<h2 class="box-title">4. 대응방안 이행 결과 등록</h2>
								</div>
								<div class="wrap-table">
									<table>
										<colgroup>
										...
										<tbody>
											<tr>
												<th scope="row">이행 현황</th>
												<td>
													<select name="sch_exe_yn" id="sch_exe_yn" class="form-control w100" disabled>
											...
											<tr>
												<th scope="row">이행 완료일</th>
												<td class="form-inline">
													<div class="input-group">
														<input type="text" id="act_cpl_dt" name="act_cpl_dt" class="form-control w90" value="" readonly>
														<span class="input-group-btn">
															<button class="btn btn-default ico" type="button" id="calendarButton3" onclick="showCalendar('yyyy-MM-dd','act_cpl_dt');" style="display:none"><i class="fa fa-calendar"></i><span class="blind">날짜 입력</span></button>
														</span>
													</div>
												</td>
											</tr>
											<tr>
												<th scope="row">이행결과 종합 의견</th>
												<td>
													<textarea id="exe_rzt_cntn" name="exe_rzt_cntn" class="form-control" placeholder="대응방안 이행 결과에 대한 종합 의견을 기술하시오." readonly></textarea>
												</td>
											</tr>
										</tbody>
									</table>
