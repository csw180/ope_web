<%--
/*---------------------------------------------------------------------------
 Program ID   : ORRC0107.jsp
 Program name : 평가대상 리스크 선정
 Description  : 화면정의서 RCSA-06
 Programer    : 박승윤
...
for(int i=0;i<vRkpTpcLst.size();i++){
	HashMap hMap = (HashMap)vRkpTpcLst.get(i);
	RkpTpc += (String)hMap.get("intgc") + "|";
	RkpTpnm += (String)hMap.get("intg_cnm") + "|";
}
	...
	<script>
		...
		function initIBSheet() {
			...
			initData.Cols = [
				...
				{Header:"업무프로세스 LV1",Type:"Text",Width:100,Align:"Left",SaveName:"prssnm1",MinWidth:50,Edit:0},
				{Header:"업무프로세스 LV2",Type:"Text",Width:100,Align:"Left",SaveName:"prssnm2",MinWidth:50,Edit:0},
				{Header:"업무프로세스 LV3",Type:"Text",Width:100,Align:"Left",SaveName:"prssnm3",MinWidth:50,Edit:0},
				{Header:"업무프로세스 LV4",Type:"Text",Width:200,Align:"Left",SaveName:"prssnm4",MinWidth:50,Edit:0},
				{Header:"업무프로세스코드",Type:"Text",Width:0,Align:"Center",SaveName:"prss",MinWidth:0, Hidden:true},
				{Header:"리스크 사례",Type:"Text",Width:200,Align:"Left",SaveName:"rk_isc_cntn",MinWidth:60,Wrap:1,MultiLineText:1,Edit:0},
				{Header:"통제활동",Type:"Text",Width:200,Align:"Left",SaveName:"cp_cntn",MinWidth:60,Wrap:1,MultiLineText:1,Edit:0},
				...
				{Header:"중요리스크",Type:"Text",Width:20,Align:"Center",SaveName:"krk_yn",MinWidth:60,Wrap:1,Edit:0},
				{Header:"평가주기",Type:"Text",Width:20,Align:"Center",SaveName:"rpt_fq_dscnm",MinWidth:60,Wrap:1,MultiLineText:1,Edit:0},
				{Header:"평가대상",Type:"Text",Width:60,Align:"Center",SaveName:"evl_obj_yn",MinWidth:60,Edit:0}
			];
		...
		function doAction(sAction) {
			...
			switch(sAction) {
				case "search":  //데이터 조회
					...
					$("form[name=ormsForm] [name=process_id]").val("ORRC010702");
					<%
						for(int i=0;i<VRkpEvlScLst.size();i++){
							HashMap hMap = (HashMap)VRkpEvlScLst.get(i);
					%>
							if($("#bas_ym").val()=="<%=(String)hMap.get("bas_ym")%>")
							{
								$("#rk_evl_prg_stsc").val("<%=(String)hMap.get("rk_evl_prg_stsc")%>");
							}				
					<%
						}
					%>
				...
				case "down2excel":
					var params = {ExcelHeaderRowHeight:25, ExcelRowHeight:17, ExcelFontSize:10, FileName : "RCSA평가대상 리스크 선정.xlsx", SheetName : "Sheet1", Merge:1} ;
 ...
 */
		...
		function doSave() {
			...
			}else if($("#rk_evl_prg_stsc").val()=="01") 
			{
				...
				mySheet.DoSave(url, { Param : "method=Main&commkind=rsa&process_id=ORRC010703&bas_ym="+$("#bas_ym").val()+"&input_ds="+$("#input_ds").val(), Col : 0 });
			}
		}
	...
	<div class="container">
		...
		<div class="content">
			...
			<form id="ormsForm" name="ormsForm">
				...
				<input type="hidden" id="rk_evl_prg_stsc" name="rk_evl_prg_stsc" />
						...
						<table>
							<tbody>
								<tr>
									...
									<td>
										<div class="select w210">
											<select class="form-control" id="bas_ym" name="bas_ym" onkeypress="EnterkeySubmit();">
<%
	for(int i=0;i<VRkpEvlScLst.size();i++){
		HashMap hMap = (HashMap)VRkpEvlScLst.get(i);
%>
												<option value="<%=(String)hMap.get("bas_ym")%>"><%=(String)hMap.get("bas_ym_nm")%></option>
<%
	}
%>
									...
									<td>
											<span class="select">
												<select class="form-control" id="sch_rkp_fq" name="sch_rkp_fq" onkeypress="EnterkeySubmit();" >
...
%>
								...
								<tr>
									...
									<td>
										<span class="select">
											<select class="form-control" id="sch_evl_obj_yn" name="sch_evl_obj_yn" onkeypress="EnterkeySubmit();">
												<option value="">전체</option>
												<option value="Y">Y</option>
												<option value="N">N</option>
											</select>
								...
								<tr>
									...
									<td>
										<div class="input-group w200">
											...
											<input type="text" class="form-control" id="sch_brnm" name="sch_brnm"  onKeyPress="EnterkeySubmitOrg('sch_brnm', 'orgSearchEnd');" readonly   placeholder="전체" onkeypress="EnterkeySubmit();"/>
											<span class="input-group-btn">
			...
			<div class="box box-grid">
				...
				<div class="box-footer">
					<div class="btn-wrap">						
						<button type="button" id="eval" class="btn btn-default" onClick="doDczProc('evl');" >평가대상등록</button>
						<button type="button" id="expt" class="btn btn-normal" onClick="doDczProc('exc');"  >평가대상제외</button>
						<!--  <button type="button" id="reqt" class="btn btn-primary" onClick="doDczProc('sub');" style="display:none">상신</button>
						
						<button type="button" id="sign" class="btn btn-primary" onClick="doDczProc('dcz');" style="display:none">결재</button>
						<button type="button" id="retn" class="btn btn-default" onClick="doDczProc('ret');" style="display:none">반려</button>-->
					</div><!-- .box-footer //-->   
				 </div>
			</div>
<!-- 			 <div class="box">
