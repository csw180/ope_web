<%--
/*---------------------------------------------------------------------------
 Program ID   : ORRC0119.jsp
 Program name : 결과보고서(전부서)
 Description  : 화면정의서 RCSA-12
 Programer    : 박승윤
...
<head>
	...
	function initIBSheet1() {
		...
		initData.Cols = [
			{Header:"평가조직",Type:"Text",Width:100,Align:"Left",SaveName:"brnm",MinWidth:60},
	...
	function mySheet1_doAction(str){
		switch(str){
			case 'search':
				...
				$("form[name=ormsForm] [name=process_id]").val("ORRC011904");
	...
	function initIBSheet2() {
		...
		initData.Cfg = {FrozenCol:0,MergeSheet:msHeaderOnly,AutoFitColWidth:"init|search|resize|rowtransaction|colhidden" }; 
		initData.Cols = [
			{Header:"평가조직|평가조직",Type:"Text",Width:100,Align:"Left",SaveName:"sheet2_brnm",MinWidth:100,Edit:0},
	...
	function mySheet2_doAction(str){
		switch(str){
			case 'search':
				...
				$("form[name=ormsForm] [name=method]").val("Main");
				$("form[name=ormsForm] [name=commkind]").val("rsa");
				$("form[name=ormsForm] [name=process_id]").val("ORRC011905");
	...
	function mySheet3_doAction(str){
		switch(str){
			case 'search':
				...
				$("form[name=ormsForm] [name=method]").val("Main");
				$("form[name=ormsForm] [name=commkind]").val("rsa");
				$("form[name=ormsForm] [name=process_id]").val("ORRC011906");
	...
	function bas_mm_chg(){
			...
			WP.setParameter("process_id", "ORRC011902");
			WP.setForm(f);
...
//						alert(result.rtnCode);
	...
	function getEvlInfo(){
		...
		WP.setParameter("method", "Main");
		WP.setParameter("commkind", "rsa");
		WP.setParameter("process_id", "ORRC011903");
		WP.setForm(f);
	...
	<div class="container">
		...
		<div class="content">
			<form name="ormsForm" method="post">
				...
				<input type="hidden" id="sch_hd_brc" name="sch_hd_brc" value=""/>
