<%--
/*---------------------------------------------------------------------------
 Program ID   : ORKR0202.jsp
 Program name : KRI > RI풀 관리 > RI조회(지주) > 신규등록
 Description  : 
 Programer    : 권성학
...
<%
	...
	Vector vRkiAttrLst = CommUtil.getCommonCode(request, "RKI_ATTR_C"); // 리스크지표속성코드
	if(vRkiAttrLst==null) vRkiAttrLst = new Vector();
	Vector vColFqLst = CommUtil.getCommonCode(request, "COL_FQ");		//수집주기
	if(vColFqLst==null) vColFqLst = new Vector();
	Vector vRptFqLst = CommUtil.getCommonCode(request, "RPT_FQ_DSC");		//보고주기
	if(vRptFqLst==null) vRptFqLst = new Vector();
	Vector vRkiUntLst= CommUtil.getCommonCode(request, "RKI_UNT_C");		//단위
	...
	Vector vMaxRkiIdLst= CommUtil.getResultVector(request, "grp01", 0, "unit06", 0, "vList");	//공통RI ID
	if(vMaxRkiIdLst==null) vMaxRkiIdLst = new Vector();
	
	HashMap hp = (HashMap)vMaxRkiIdLst.get(0);
	String max_comn_rki_id = (String)hp.get("max_comn_rki_id");
%>
<!DOCTYPE html>
<html lang="ko">
<head>
...
<body>
	<div id="" class="popup modal block">
		<div class="p_frame w1100">
			...
			<div id="mpp_area"></div>
			<div class="p_head">
				<h3 class="title">공통 RI 신규등록(지주)</h3>
			</div>
			<div class="p_body">
				<div class="p_wrap">
						...
						<table>
							...
							<tbody>
								<tr>
									<th>공통RI-ID</th>
									<td>
										<input type="text" name="comn_rki_id" id="comn_rki_id" class="form-control" value="<%=max_comn_rki_id%>" disabled>
									</td>
									<th>지표명</th>
									<td colspan="3">
										<input type="text" class="form-control" id="comn_rkinm" name="comn_rkinm" maxlength="100">
									</td>
									<th>KRI 속성</th>
									<td>
										<div class="select">
...
%>
								...
								<tr>
									<th>지표목적</th>
									<td colspan="7">
										<input type="text" name="rki_obv_cntn" id="rki_obv_cntn" class="form-control" maxlength="1000">
									</td>
								</tr>
								<tr>
									<th>지표정의</th>
									<td colspan="7">
										<textarea name="rki_def_cntn" id="rki_def_cntn" class="textarea h100" maxlength="500"></textarea>
									</td>
								</tr>
								<tr>
									<th>지표산식</th>
									<td colspan="7">
										<textarea name="idx_fml_cntn" id="idx_fml_cntn" class="textarea h100" maxlength="500"></textarea>
									</td>
								</tr>
							</tbody>
						</table>
					...
					<div class="box box-grid mt20">
						...
						<div class="btn-wrap center ph15">
	...
	<script>
		$(document).ready(function(){
			...
			$(".btn-close").click( function(event){
				...
				parent.$("#ifrRkiAdd").attr("src","about:blank");
				event.preventDefault();
			});
		});
			
		function closePop(){
			$("#winRkiAdd",parent.document).hide();
			parent.$("#ifrRkiAdd").attr("src","about:blank");
		}
	</script>
</body>
