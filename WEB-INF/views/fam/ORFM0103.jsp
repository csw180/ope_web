<%--
/*---------------------------------------------------------------------------
 Program ID   : ORFM0103.jsp
 Program name : 평판리스크 지표 상세조회 팝업
 Description  : 
 Programmer   : 김병현
...
<head>
	...
	<script>
		...
		function initIBSheet() {
			...
			initData.Cols = [
				{Header:"선택",			Type:"CheckBox",Width:50,Align:"Center",SaveName:"ischeck",Wrap:1,MinWidth:50,Edit:1},
				...
				{Header:"주관부서",		Type:"Text",Width:150,Align:"Center",SaveName:"br_nm",Wrap:1,MinWidth:20,Edit:0},
			];
		...
		function doAction(sAction) {
			switch(sAction) {
				case "search":  //데이터 조회	
					...
					$("form[name=ormsForm] [name=process_id]").val("ORFM010302");
					...
					WP.load(url, inputData,
					{
						success: function(result){
							if(result!='undefined' && result.rtnCode=="S") {
								alert("삭제되었습니다.");
								//mySheet3.RowDelete(srow, 0);
								mySheet3.SetSelectRow(-1);
								init();
								doAction("search"); 
				...
				case "save":
					...
					WP.setParameter("method", "Main");
					WP.setParameter("commkind", "fam");
					WP.setParameter("process_id", "ORFM010303");
					WP.setForm(f);
				...
				case "edit":
					...
					$("#sch_sup_brc").attr("disabled",false);
					$("#sch_kri_yn").attr("disabled",false);
					break; 
			}
		}
		function hideSheet(){
			if($("#sch_kri_yn").val() == 'N') {
				$("#sheet_div").hide();
				var last_row = mySheet3.GetDataLastRow();
				if(mySheet3.GetDataFirstRow()>=0){
				 	if(!confirm("등록된 연관 KRI를 삭제하시겠습니까?")) return;
					for(var j=mySheet3.GetDataFirstRow(); j<=last_row; j++){
						mySheet3.RowDelete();
					}
				} 
			}
...
<body onkeyPress="return EnterkeyPass()">
			...
			<div class="p_body"> 
				<div class="p_wrap">  
					<form name="ormsForm" method="post">
						...
						<input type="hidden" id="temp" name="temp" value="<%=(String)hMap.get("br_nm")%>"/>                      
						<div id="hdng_area"></div>  
						<div id="brcd_area"></div>
					
						<section class="box-grid">
							<div class="box-header">
								<h2 class="box-title">평판지표 상세조회</h2>
								<div class="area-tool">
									<button type="button" class="btn btn-default btn-xs" onclick="doAction('edit');"><i class="fa fa-pencil"></i><span class="txt">수정</span></button>
								</div>
							</div>	
								...
								<table>
									...
									<tbody>
										...
										<tr>
											<th>유형 구분</th>
											<td>
												<select class="form-control"  id="sch_typnm" name="sch_typnm" disabled>
	...
	%>
											...
											<th>주관 부서</th>
											<td colspan="3">
												<select class="form-control"  id="sch_sup_brc" name="sch_sup_brc" disabled>
													<option value="">전체</option>
	<%
	...
	%>
										...
										<tr>
											<th>지표 산식</th>
											<td colspan="5">
												<textarea id="sch_fml_cntn" name="sch_fml_cntn" cols="20" rows="3" class="form-control" disabled><%=StringUtil.htmlEscape((String)hMap.get("fml_cntn"),false,false) %></textarea>
											</td>
										</tr>
										<tr>
											<th>수집주기</th>
											<td>
												<select class="form-control"  id="sch_fq_dsc" name="sch_fq_dsc" disabled>
													<option value="">전체</option>
	<%
	...
	%>
											...
											<th>지표 추출 방식</th>
											<td>
												<select class="form-control" id = "sch_psb_yn" name = "sch_psb_yn" disabled>
<% 
if(((String)hMap.get("psb_yn")).equals("전산")) { 
%>
													<option value="Y" selected>전산</option>
													<option value="N">수기</option>
<%
} else {
%>
													<option value="Y">전산</option>
													<option value="N" selected>수기</option>
<%
} 
%>
												</select>
											...
											<th>기존 KRI 여부</th>
											<td>
												<select class="form-control" id="sch_kri_yn" name="sch_kri_yn" onchange="hideSheet()" disabled>
<% 
...
%>
							...
							<div id="sheet_div">
<%} else{ %>
							<div  id="sheet_div" style="display:none">
<% }%>
								<div class="box-header">
									<h3 class="box-title">연관 KRI 등록</h3>
									<div class="area-tool">
