<%--
/*---------------------------------------------------------------------------
 Program ID   : ORCO0105.jsp
 Program name : 공통 > 영향유형(팝업)
 Description  : 
 Programer    : 권성학
...
<head>
	...
	<script>
		...
		function initIBSheet() {
			...
			initData.Cols = [
 				{Header:"상위영향유형코드",	Type:"Text",	SaveName:"up_ifn_tpc",		Hidden:true},
 				{Header:"영향유형코드",		Type:"Text",	SaveName:"ifn_tpc",			Hidden:true},
 				{Header:"레벨",			Type:"Text",	SaveName:"level",			Hidden:true},
 				{Header:"영향유형명",		Type:"Text",	SaveName:"ifn_tpnm",		TreeCol:1,	Width:440,	MinWidth:150},
 				{Header:"영향유형코드1레벨",	Type:"Text",	SaveName:"ifn_tpc_lv1",		Hidden:true},
 				{Header:"영향유형명1레벨",	Type:"Text",	SaveName:"ifn_tpnm_lv1",	Hidden:true},
 			];
		...
		function mySheet_OnDblClick(Row, Col, Value, CellX, CellY, CellW, CellH) { 
			if(Row >= mySheet.GetDataFirstRow()){
				...
				if(parent.IFN2_ONLY != null && parent.IFN2_ONLY && $("#level").val() != "2" ){
					alert("2레벨을 선택하세요.");
					return;
				}
				parent.ifnSearchEnd(
					$("#ifn_tpc").val(), $("#ifn_tpnm").val()
						,$("#ifn_tpc_lv1").val(), $("#ifn_tpnm_lv1").val()
				);
		...
		function mySheet_OnSelectCell(OldRow, OldCol, NewRow, NewCol,isDelete) {
			...
			if(NewRow >= mySheet.GetDataFirstRow()){
				...
				$("#ifn_tpc").val(mySheet.GetCellValue(NewRow, "ifn_tpc"));
				$("#ifn_tpnm").val(mySheet.GetCellValue(NewRow, "ifn_tpnm"));
				$("#ifn_tpc_lv1").val(mySheet.GetCellValue(NewRow, "ifn_tpc_lv1"));
				$("#ifn_tpnm_lv1").val(mySheet.GetCellValue(NewRow, "ifn_tpnm_lv1"));
			}
		}
			...
			switch(sAction) {
				case "search":  //데이터 조회
					...
					if(parent.ROLE_ID == undefined || parent.ROLE_ID == null || parent.ROLE_ID != "hcorm"){
						$("form[name=ormsForm] [name=process_id]").val("ORCO010502");
					}else{
						$("form[name=ormsForm] [name=sch_grp_org_c]").val($("#sch_grp_org_c",parent.document).val());
						$("form[name=ormsForm] [name=process_id]").val("ORCO010592");
					}
		...
		function cur_click() {
			...
			if(parent.CUR_IFN_TPC!=null && parent.CUR_IFN_TPC!=""){
				if(mySheet.GetDataFirstRow()>=0){
					for(var j=mySheet.GetDataFirstRow(); j<=mySheet.GetDataLastRow(); j++){
						if(mySheet.GetCellValue(j, "ifn_tpc")==parent.CUR_IFN_TPC){
...
<body onkeyPress="return EnterkeyPass()">
	...
	<input type="hidden" id="ifn_tpnm" name="ifn_tpnm" /> <!-- 선택한 영향유형명 -->
	<input type="hidden" id="ifn_tpc" name="ifn_tpc" /> <!-- 선택한 영향유형코드 -->
	<input type="hidden" id="level" name="level" /> <!-- 선택한 level -->
	<input type="hidden" id="ifn_tpnm_lv1" name="ifn_tpnm_lv1" /> <!-- 선택한 영향유형명lv1 -->
	<input type="hidden" id="ifn_tpc_lv1" name="ifn_tpc_lv1" /> <!-- 선택한 영향유형코드lv1 -->
	<input type="hidden" id="path" name="path" />
	<input type="hidden" id="process_id" name="process_id" />
	<input type="hidden" id="commkind" name="commkind" />
	<input type="hidden" id="method" name="method" />
	<input type="hidden" id="rtn_ifn_c" name="rtn_ifn_c" />
	<input type="hidden" id="rtn_ifn_nm" name="rtn_ifn_nm" />
