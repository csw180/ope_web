...
<head>
	...
	/*Sheet 기본 설정 */
		function initIBSheet() {
			...
			initData.Cols = [
				...
				{Header:"선택|선택",							Type:"CheckBox",		MinWidth:30,	Align:"Left",		SaveName:"ischeck", 		Edit:true},
				{Header:"Code",								Type:"Text",			MinWidth:60,	Align:"Left",		SaveName:"bsn_prss_c",  Edit:false, Wrap:true, Hidden:true},
				{Header:"LV1|Code",							Type:"Text",			MinWidth:60,	Align:"Left",		SaveName:"lv1_bsn_prss_c",  Edit:false, Wrap:true},
				{Header:"LV1|프로세스",						Type:"Text",			MinWidth:60,	Align:"Center",		SaveName:"lv1_bsn_prsnm",	Edit:false, Wrap:true},
				{Header:"LV2|Code",							Type:"Text",			MinWidth:60,	Align:"Center",		SaveName:"lv2_bsn_prss_c",  Edit:false, Wrap:true},			
				{Header:"LV2|프로세스",						Type:"Text",			MinWidth:60,	Align:"Center",		SaveName:"lv2_bsn_prsnm", 	Edit:false, Wrap:true},
				{Header:"LV3|Code",							Type:"Text",			MinWidth:60,	Align:"Center",		SaveName:"lv3_bsn_prss_c",	Edit:false, Wrap:true},
				{Header:"LV3|프로세스",						Type:"Text",			MinWidth:60,	Align:"Left",		SaveName:"lv3_bsn_prsnm", 	Edit:false, Wrap:true},
				{Header:"LV4|Code",							Type:"Text",			MinWidth:60,	Align:"Left",		SaveName:"lv4_bsn_prss_c",  Edit:false, Wrap:true},
				{Header:"LV4|프로세스",						Type:"Text",			MinWidth:60,	Align:"Center",		SaveName:"lv4_bsn_prsnm",	Edit:false, Wrap:true}		
			];
		...
		function mySheet_OnSearchEnd(code, message) {
			...
			if(code != 0) {
				...
				for(var i = parent.mySheet2.GetDataFirstRow(); i<= parent.mySheet2.GetDataLastRow(); i++){
					for(var j = mySheet.GetDataFirstRow(); j<=mySheet.GetDataLastRow(); j++){
						if(mySheet.GetCellValue(j, "bsn_prss_c")==parent.mySheet2.GetCellValue(i, "bsn_prss_c")){
	...
	function doAction(sAction) {
		switch(sAction) {
			case "Search":  //부서 조회
				...
				$("form[name=ormsForm] [name=process_id]").val("ORAD011103");
	...
	function prss_save(){
		...
		if(mySheet.GetDataFirstRow()>=0){
			for(var j=mySheet.GetDataFirstRow(); j<=mySheet.GetDataLastRow(); j++){
				if(mySheet.GetCellValue(j,"ischeck")==1){
					add_html += "<input type='hidden' name='sch_bsn_prss_c' value='" + mySheet.GetCellValue(j,"bsn_prss_c") + "'>";
				}
			}
		}	
		if(add_html==""){
			alert("추가할 업무프로세스를 선택해 주세요..");
			return;
		}
		...
		var f = document.ormsForm;
		if(!confirm("선택한 업무프로세스를 추가하시겠습니까?")) return;
			
		WP.clearParameters();
		WP.setParameter("method", "Main");
		WP.setParameter("commkind", "adm");
		WP.setParameter("process_id", "ORAD011102"); 
		WP.setForm(f);
		...
		WP.load(url, inputData,
		    {
			  success: function(result){
				  if(result != 'undefined' && result.rtnCode== 'S'){
					  alert(result.rtnMsg);
					  parent.doAction('brc_prss_Search');
				  }
