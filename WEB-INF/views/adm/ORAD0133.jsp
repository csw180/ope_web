<%--
/*---------------------------------------------------------------------------
 Program ID   : ORAD0133.jsp
 Program name : ADMIN > 배치 > 배치작업관리 > 수정
 Description  : 
 Programer    : 권성학
...
<%
	...
	Vector vBtwkInfoLst= CommUtil.getResultVector(request, "grp01", 0, "unit04", 0, "vList");	//배치작업상세정보
	if(vBtwkInfoLst==null) vBtwkInfoLst = new Vector();
	Vector vPrdBtwkLst= CommUtil.getResultVector(request, "grp01", 0, "unit05", 0, "vList");	//선행배치목록
	if(vPrdBtwkLst==null) vPrdBtwkLst = new Vector();
	
	HashMap hBtwkMap = null;
	if(vBtwkInfoLst.size()>0){
		hBtwkMap = (HashMap)vBtwkInfoLst.get(0);
	}else{
		hBtwkMap = new HashMap();
	}
	
	JSONObject jsonObj = new JSONObject();
	JSONArray jsonlist = new JSONArray();
	
	HashMap<String, String> hPrdBtwkMap = new HashMap<String, String>();
	
	for(int i=0; i<vPrdBtwkLst.size(); i++){
		hPrdBtwkMap = (HashMap)vPrdBtwkLst.get(i);
		jsonlist.add(hPrdBtwkMap);
	}
...
<script language="javascript">
	...
	$(document).ready(function(){
		...
		$("#btwk_id").attr("disabled",true);
		<%if("N".equals((String)hBtwkMap.get("mft_yn"))){%>
	...
	function initIBSheet2() {
		...
		for(var i=1; i<=10; i++){
			if($("#a_imp_btwk_id"+i).val() == "") break;
			
			var row = mySheet2.DataInsert(999);

			mySheet2.SetCellValue(row, "btwk_id", $("#a_imp_btwk_id"+i).val());
			mySheet2.SetCellValue(row, "btwknm", $("#a_imp_btwk_nm"+i).val());
			mySheet2.SetCellValue(row, "btwk_exe_fq_dsnm", "<%=(String)hBtwkMap.get("btwk_exe_fq_dsnm")%>");
			mySheet2.SetCellValue(row, "btwk_exe_fq_dsc", "<%=(String)hBtwkMap.get("btwk_exe_fq_dsc")%>");
		}
	...
	function delBtwk(){
		...
		WP.setParameter("method", "Main");
		WP.setParameter("commkind", "adm");
		WP.setParameter("process_id", "ORAD013103");
		WP.setForm(f);
...
<body style="background-color:transparent">
	<div id="" class="popup modal block">
		<div class="p_frame w1100">
			...
			<input type="hidden" id="a_imp_btwk_id1" name="a_imp_btwk_id1" value="<%=(String)hBtwkMap.get("imp_btwk_id1")%>"/>
			<input type="hidden" id="a_imp_btwk_id2" name="a_imp_btwk_id2" value="<%=(String)hBtwkMap.get("imp_btwk_id2")%>"/>
			<input type="hidden" id="a_imp_btwk_id3" name="a_imp_btwk_id3" value="<%=(String)hBtwkMap.get("imp_btwk_id3")%>"/>
			<input type="hidden" id="a_imp_btwk_id4" name="a_imp_btwk_id4" value="<%=(String)hBtwkMap.get("imp_btwk_id4")%>"/>
			<input type="hidden" id="a_imp_btwk_id5" name="a_imp_btwk_id5" value="<%=(String)hBtwkMap.get("imp_btwk_id5")%>"/>
			<input type="hidden" id="a_imp_btwk_id6" name="a_imp_btwk_id6" value="<%=(String)hBtwkMap.get("imp_btwk_id6")%>"/>
			<input type="hidden" id="a_imp_btwk_id7" name="a_imp_btwk_id7" value="<%=(String)hBtwkMap.get("imp_btwk_id7")%>"/>
			<input type="hidden" id="a_imp_btwk_id8" name="a_imp_btwk_id8" value="<%=(String)hBtwkMap.get("imp_btwk_id8")%>"/>
			<input type="hidden" id="a_imp_btwk_id9" name="a_imp_btwk_id9" value="<%=(String)hBtwkMap.get("imp_btwk_id9")%>"/>
			<input type="hidden" id="a_imp_btwk_id10" name="a_imp_btwk_id10" value="<%=(String)hBtwkMap.get("imp_btwk_id10")%>"/>
			<input type="hidden" id="a_imp_btwk_nm1" name="a_imp_btwk_nm1" value="<%=(String)hBtwkMap.get("imp_btwk_nm1")%>"/>
			<input type="hidden" id="a_imp_btwk_nm2" name="a_imp_btwk_nm2" value="<%=(String)hBtwkMap.get("imp_btwk_nm2")%>"/>
			<input type="hidden" id="a_imp_btwk_nm3" name="a_imp_btwk_nm3" value="<%=(String)hBtwkMap.get("imp_btwk_nm3")%>"/>
			<input type="hidden" id="a_imp_btwk_nm4" name="a_imp_btwk_nm4" value="<%=(String)hBtwkMap.get("imp_btwk_nm4")%>"/>
			<input type="hidden" id="a_imp_btwk_nm5" name="a_imp_btwk_nm5" value="<%=(String)hBtwkMap.get("imp_btwk_nm5")%>"/>
			<input type="hidden" id="a_imp_btwk_nm6" name="a_imp_btwk_nm6" value="<%=(String)hBtwkMap.get("imp_btwk_nm6")%>"/>
			<input type="hidden" id="a_imp_btwk_nm7" name="a_imp_btwk_nm7" value="<%=(String)hBtwkMap.get("imp_btwk_nm7")%>"/>
			<input type="hidden" id="a_imp_btwk_nm8" name="a_imp_btwk_nm8" value="<%=(String)hBtwkMap.get("imp_btwk_nm8")%>"/>
			<input type="hidden" id="a_imp_btwk_nm9" name="a_imp_btwk_nm9" value="<%=(String)hBtwkMap.get("imp_btwk_nm9")%>"/>
			<input type="hidden" id="a_imp_btwk_nm10" name="a_imp_btwk_nm10" value="<%=(String)hBtwkMap.get("imp_btwk_nm10")%>"/>
			<div id="grid_area"></div>
			<div class="p_head">
				<h3 class="title">배치작업 상세조회</h3>
			</div>
			<div class="p_body">
							...
							<table>
								...
								<tbody>
									<tr>
										...
										<td>
											<div class="select">
												<select name="grp_org_c" id="grp_org_c" class="form-control" readonly>
													<option value="">선택</option>
<%
		for(int i=0;i<vAllGrpList.size();i++){
			HashMap hMap1 = (HashMap)vAllGrpList.get(i);
			if(((String)hMap1.get("grp_org_c")).equals((String)hBtwkMap.get("grp_org_c"))){
%>
												<option value="<%=(String)hMap1.get("grp_org_c")%>" selected><%=(String)hMap1.get("grp_orgnm")%></option>
<%		
			}else{
%>
												<option value="<%=(String)hMap1.get("grp_org_c")%>"><%=(String)hMap1.get("grp_orgnm")%></option>
<%
			}
		}
%>
										...
										<th>배치작업ID</th>
										<td>
											<input type="text" name="btwk_id" id="btwk_id" class="form-control" maxlength="19" readonly value="<%=(String)hBtwkMap.get("btwk_id")%>">
										</td>
										<th>배치작업명</th>
										<td colspan="3">
											<input type="text" name="btwknm" id="btwknm" class="form-control" maxlength="50" value="<%=(String)hBtwkMap.get("btwknm")%>">
										</td>
									</tr>
									<tr>
										<th>실행주기</th>
										<td>
											<div class="select">
												<select name="btwk_exe_fq_dsc" id="btwk_exe_fq_dsc" class="form-control">
													<option value="">선택</option>
<%
		for(int i=0;i<vBtwkExeFqLst.size();i++){
			HashMap hMap2 = (HashMap)vBtwkExeFqLst.get(i);
			if(((String)hMap2.get("intgc")).equals((String)hBtwkMap.get("btwk_exe_fq_dsc"))){
%>
...
<%
		for(int i=0;i<vHldyWkExeLst.size();i++){
			...
			if(((String)hMap3.get("intgc")).equals((String)hBtwkMap.get("hldy_wk_exe_dsc"))){
%>
...
%>
										...
										<td>
											<div class="select">
												<select name="mft_yn" id="mft_yn" class="form-control" onchange="openMft();">
													<option value="">선택</option>
													<option value="Y" <%if("Y".equals((String)hBtwkMap.get("mft_yn"))){%>selected<%}%>>Y</option>
													<option value="N" <%if("N".equals((String)hBtwkMap.get("mft_yn"))){%>selected<%}%>>N</option>
												</select>
											</div>
										</td>
										<th>MFT ID</th>
										<td>
											<input type="text" name="mft_id" id="mft_id" class="form-control" maxlength="20" value="<%=(String)hBtwkMap.get("mft_id")%>">
										</td>
									</tr>
									<tr>
										<th>기준일구분</th>
										<td>
											<div class="select">
												<select name="btwk_basdt_dsc" id="btwk_basdt_dsc" class="form-control" >
													<option value="9" <%if("9".equals((String)hBtwkMap.get("btwk_basdt_dsc"))){%>selected<%}%>>없음</option>
													<option value="1" <%if("1".equals((String)hBtwkMap.get("btwk_basdt_dsc"))){%>selected<%}%>>YYYYMMDD</option>
													<option value="2" <%if("2".equals((String)hBtwkMap.get("btwk_basdt_dsc"))){%>selected<%}%>>YYYYMM</option>
													<option value="3" <%if("3".equals((String)hBtwkMap.get("btwk_basdt_dsc"))){%>selected<%}%>>YYMMDD</option>
													<option value="4" <%if("4".equals((String)hBtwkMap.get("btwk_basdt_dsc"))){%>selected<%}%>>YYMM</option>
												</select>
											</div>
										</td>
										<th>파일입수방식</th>
										<td>
											<div class="select">
												<select name="tms_fl_path_dsc" id="tms_fl_path_dsc" class="form-control" >
													<option value="" <%if("".equals((String)hBtwkMap.get("tms_fl_path_dsc"))){%>selected<%}%>>사용안함</option>
													<option value="1" <%if("1".equals((String)hBtwkMap.get("tms_fl_path_dsc"))){%>selected<%}%>>MFT</option>
													<option value="2" <%if("2".equals((String)hBtwkMap.get("tms_fl_path_dsc"))){%>selected<%}%>>SAM</option>
												</select>
											</div>
										</td>
										<td colspan="4">
									...
									<tr>
										<th>배치실행경로</th>
										<td colspan="7">
											<input type="text" name="bat_exe_pathnm" id="bat_exe_pathnm" class="form-control" maxlength="100" value="<%=(String)hBtwkMap.get("bat_exe_pathnm")%>">
										</td>
									</tr>
									<tr>
										<th>MFT파일경로</th>
										<td colspan="7">
											<input type="text" name="surc_fl_pathnm" id="surc_fl_pathnm" class="form-control" maxlength="100" value="<%=(String)hBtwkMap.get("surc_fl_pathnm")%>">
										</td>
									</tr>
									<tr>
										<th>MFT파일명</th>
										<td colspan="5">
											<input type="text" name="surc_flnm" id="surc_flnm" class="form-control" maxlength="100" value="<%=(String)hBtwkMap.get("surc_flnm")%>">
										</td>
										<th>체크파일 확장자명</th>
										<td>
											<input type="text" name="wk_chk_fl_xcrnm" id="wk_chk_fl_xcrnm" class="form-control" maxlength="3" value="<%=(String)hBtwkMap.get("wk_chk_fl_xcrnm")%>">
										</td>
									</tr>
									<tr>
										<th>SAM파일경로</th>
										<td colspan="5">
											<input type="text" name="trgt_fl_pathnm" id="trgt_fl_pathnm" class="form-control" maxlength="100" value="<%=(String)hBtwkMap.get("trgt_fl_pathnm")%>">
										</td>
										<th>SAM파일 확장자명</th>
										<td>
											<input type="text" name="wk_sfile_xcrnm" id="wk_sfile_xcrnm" class="form-control" maxlength="3" value="<%=(String)hBtwkMap.get("wk_sfile_xcrnm")%>">
										</td>
									</tr>
									<tr>
										<th>파일대기시간(초)</th>
										<td>
											<input type="text" name="fl_stb_hr" id="fl_stb_hr" class="form-control w100 right" maxlength="10" onkeypress="inNumber()" value="<%=(String)hBtwkMap.get("fl_stb_hr")%>">
										</td>
										<th>보류일(DD)</th>
										<td>
											<input type="text" name="rzv_dd" id="rzv_dd" class="form-control right" maxlength="2" onkeypress="inNumber()" value="<%=(String)hBtwkMap.get("rzv_dd")%>">
										</td>
										<th>보류조건</th>
										<td colspan="3">
...
<%
		for(int i=0;i<vRzvMrkLst.size();i++){
			HashMap hMap4 = (HashMap)vRzvMrkLst.get(i);
			if(((String)hMap4.get("intgc")).equals((String)hBtwkMap.get("rzv_mrk_c"))){
%>
...
%>	
										...
										<td colspan="3">
											<div class="select">
												<select name="bmm_hldy_rzv_yn" id="bmm_hldy_rzv_yn" class="form-control w100">
													<option value="Y" <%if("Y".equals((String)hBtwkMap.get("bmm_hldy_rzv_yn"))){%>selected<%}%>>Y</option>
													<option value="N" <%if("N".equals((String)hBtwkMap.get("bmm_hldy_rzv_yn"))){%>selected<%}%>>N</option>
												</select>
											</div>
										</td>
										<th>실행제외여부</th>
										<td colspan="3">
											<div class="select">
												<select name="exe_x_yn" id="exe_x_yn" class="form-control w100">
													<option value="Y" <%if("Y".equals((String)hBtwkMap.get("exe_x_yn"))){%>selected<%}%>>Y</option>
													<option value="N" <%if("N".equals((String)hBtwkMap.get("exe_x_yn"))){%>selected<%}%>>N</option>
												</select>
											</div>
										</td>
									</tr>
			...
			<div class="p_foot">
				<div class="btn-wrap center">
					...
					<button type="button" class="btn btn-primary" onclick="delBtwk();">삭제</button>
	...
	<script>
		$(document).ready(function(){
			...
			$(".btn-close").click( function(event){
				...
				parent.$("#ifrBtwkSchdMod").attr("src","about:blank");
				event.preventDefault();
			});
		});
			
		function closePop(){
			$("#winBtwkSchdMod",parent.document).hide();
			parent.$("#ifrBtwkSchdMod").attr("src","about:blank");
		}
	</script>
</body>
