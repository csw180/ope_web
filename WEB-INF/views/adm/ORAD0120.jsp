<%--
/*---------------------------------------------------------------------------
 Program ID   : ORAD0120.jsp
 Program name : ADMIN > 사용자/권한관리 > 프로세스 권한 조회
 Description  : 
 Programer    : 권성학
...
<html lang="ko">
		...
		<script>
			...
			function initIBSheet() {
				...
				initData.Cols = [
				    ...
				    {Header:"권한그룹명",Type:"Combo",Width:300,Align:"Left",SaveName:"auth_grp_id",MinWidth:110,Edit:false,ComboText:"<%=auth_grp_id_nm%>",ComboCode:"<%=auth_grp_id%>"},
				    {Header:"프로세스그룹",Type:"Combo",Width:200,Align:"Left",SaveName:"prss_kdc",MinWidth:40, Edit:false,ComboText:"|<%=prss_kdc_nm%>",ComboCode:"|<%=prss_kdc%>"},
					{Header:"프로세스ID",Type:"Text",Width:200,Align:"Left",SaveName:"pgid",MinWidth:40, Edit:false},
				    {Header:"권한그룹명(신)",Type:"Combo",Width:300,Align:"Left",SaveName:"new_auth_grp_id",MinWidth:110,ComboText:"<%=auth_grp_id_nm%>",ComboCode:"<%=auth_grp_id%>"},
				    {Header:"프로세스그룹(신)",Type:"Combo",Width:200,Align:"Left",SaveName:"new_prss_kdc",MinWidth:40,ComboText:"<%=prss_kdc_nm%>",ComboCode:"<%=prss_kdc%>"},
					{Header:"프로세스ID(신)",Type:"Text",Width:200,Align:"Left",SaveName:"new_pgid",MinWidth:40, Wrap:true}
				];
			...
			function mySheet_OnRowSearchEnd (Row) {
				...
				mySheet.SetCellValue(Row,"new_pgid",mySheet.GetCellValue(Row,"pgid"));
				mySheet.SetCellValue(Row,"new_prss_kdc",mySheet.GetCellValue(Row,"prss_kdc"));
			...
			function doAction(sAction) {
				switch(sAction) {
					case "search":  //데이터 조회
						...
						$("form[name=ormsForm] [name=process_id]").val("ORAD012002");
					...
					case "save":	//저장
						...
						for(var i=0; i<=arrow.length; i++){
							if(mySheet.GetCellValue(arrow[i],"new_pgid") == ""){
								alert("프로세스ID를 입력해 주십시오.");
								mySheet.SelectCell(arrow[i],"new_pgid","1");
								return;
							}
						}
						mySheet.DoSave("<%=System.getProperty("contextpath")%>/comMain.do?method=Main&commkind=adm&process_id=ORAD012003");
						break;
						
					case "down2excel":
						var params = {ExcelHeaderRowHeight:25, ExcelRowHeight:17, ExcelFontSize:10, FileName : "프로세스권한.xlsx", SheetName : "Sheet1", DownTreeHide:"True", DownCols:"2|3|4"} ;
	...
	%>
										...
										<th scope="row">프로세스그룹</th>
										<td>
											<span class="select">
												<select class="form-control w220" id="intgc" name="intgc" >
													<option value="">전체</option>
	<%
			for(int i=0;i<vLst1.size();i++){
				HashMap hp1 = (HashMap)vLst1.get(i);
	...
	%>
													<option value="<%=(String)hp1.get("intgc")%>"><%=nm%></option>
	<%
			}
	%>
												</select>
										...
										<th scope="row">프로세스ID</th>
										<td>
											<input type="text" class="form-control" id="pgid" name="pgid" placeholder="" onkeypress="EnterkeySubmit();">
										</td>
									</tr>	
								</tbody>
							</table>
