<%--
/*---------------------------------------------------------------------------
 Program ID   : ORLS0117.jsp
 Program name : 계정 및 잔액변동 모니터링 (ORM팀 손실담당자용)
 Description  : 
 Programer    : 김남원
		...
		$(document).ready(function(){
			...
			function initIBSheet() {
				...
				initData.Cols = [
				    ...
				    {Header:"회계처리일자",		Type:"Date",	SaveName:"acg_prc_dt",					MinWidth:90,	Align:"center",Format:"yyyy-MM-dd"},
				    {Header:"부서코드",			Type:"Text",	SaveName:"brc",			Hidden:true,	MinWidth:120},
				    {Header:"부서\n(영업점)",	Type:"Text",	SaveName:"brnm",						MinWidth:80,	Align:"center"},//9
				    ...
				    {Header:"계정과목명",		Type:"Text",	SaveName:"acc_sbjnm",					MinWidth:150,	Align:"Left"},
				    ...
				    {Header:"금액종류",			Type:"Combo",	SaveName:"acc_dsc",						MinWidth:100,	Align:"Center",							ComboText:"", ComboCode:"", Edit:false},//19
			...
			function doAction(sAction) {
				switch(sAction) {
					case "search":  //데이터 조회
						...
						$("form[name=ormsForm] [name=process_id]").val("ORLS011702");
...
// 				mySheet.DoSave("<%=System.getProperty("contextpath")%>/comMain.do?method=Main&commkind=los&process_id=ORLS011704&all_x_rsnctt="+$("#all_x_rsnctt").val()+"&lss_rg_stsc=03");
			...
			function regX(){
				for(var i = mySheet.GetDataFirstRow(); i<=mySheet.GetDataLastRow(); i++){
					if(mySheet.GetCellValue(i, "ischeck")=="1"){
						...
						mySheet.SetCellValue(i, "lss_rg_rqr_stsc", "02");
						mySheet.SetCellValue(i, "lss_rg_rqr_dt", "");
						mySheet.SetCellEditable(i, "x_rsnctt",1);
			...
			function save(){
				for(var i = mySheet.GetDataFirstRow(); i<=mySheet.GetDataLastRow(); i++){
					if(mySheet.GetCellValue(i, "status")=="U"){	//수정
					}	
				}
				
				mySheet.DoSave("<%=System.getProperty("contextpath")%>/comMain.do?method=Main&commkind=los&process_id=ORLS011703&role_id=orm");
			}
			...
			function regLss(){
				...
				for(var i = mySheet.GetDataFirstRow(); i<=mySheet.GetDataLastRow(); i++){
					if(mySheet.GetCellValue(i, "ischeck")=="1"){
						if(mySheet.GetCellValue(i, "status")==""&mySheet.GetCellValue(i, "lss_rg_stsc")=="01"){
			...
			function regRqr(){
				for(var i = mySheet.GetDataFirstRow(); i<=mySheet.GetDataLastRow(); i++){
					if(mySheet.GetCellValue(i, "ischeck")=="1"){
						...
						mySheet.SetCellValue(i, "status", "U");	//기요청건을 재요청할 수 있게 강제로 수정상태로 변경
						mySheet.SetCellValue(i, "lss_rg_rqr_stsc", "01");
						mySheet.SetCellValue(i, "lss_rg_rqr_dt", "<%=to_day %>");
						mySheet.SetCellValue(i, "lss_rg_stsc", "");
						mySheet.SetCellValue(i, "x_rsnctt", "");
	...
	<body class="">
			<div class="container">
				...
				<form name="ormsForm">
					...
					<input type="hidden" id="role_id" name="role_id" value="orm  " />		<!-- 계정등록시 권한 -->
...
%>
	...
	%>
										...
										<tr>
											...
											<td>
												<div class="input-group">
												  <input type="text" class="form-control w150" id="brnm" name="brnm" onKeyPress="EnterkeySubmitOrg('brnm', 'orgSearchEnd');">
				...
				%>
								...
								<div class="btn-group">
									<button type="button" class="btn btn-primary btn-xs" onclick="regLss();">사건등록</button>
								</div>
								<div class="btn-group">
									<button type="button" class="btn btn-primary btn-xs" onclick="regRqr();">등록요청</button>
									<button type="button" class="btn btn-primary btn-xs" onclick="regX();">등록불필요</button>
								</div>
