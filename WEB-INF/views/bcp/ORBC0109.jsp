...
if(ra_sc==null) ra_sc = "";
String rsk_ctl_suv_cntn = (String) form.get("hd_rsk_ctl_suv_cntn");
if("2".equals(servergubun)){
	rsk_ctl_suv_cntn = new String(rsk_ctl_suv_cntn.getBytes("ISO8859_1"), "UTF-8");
}

String chk_ra_dcz_stsc = (String) form.get("chk_ra_dcz_stsc");
if(ra_sc==null) chk_ra_dcz_stsc = "";

if(rsk_ctl_suv_cntn.equals("입력")) rsk_ctl_suv_cntn = "";
...
<head>
	...
	<script>
		...
		function insert(){
			...
			parent.mySheet2.SetCellValue(row, "rsk_ctl_suv_cntn", $("#rsk_ctl_suv_cntn").text());
		...
		function save(){
			...
			WP.setParameter("process_id", "ORBC010902");
			WP.setForm(f);
...
<body>
									...
									<table>
										...
										<tbody>
											<tr>
												<th>위험통제 현황 조사</th>
												<td>
													<textarea name="rsk_ctl_suv_cntn" id="rsk_ctl_suv_cntn" class="textarea h200"><%=rsk_ctl_suv_cntn %></textarea>
												</td>
											</tr>
										</tbody>
									</table>
	...
	<script>
		...
		function closePop(){
			$("#winRskCtl",parent.document).hide();
		}
	</script>
</body>
</html>